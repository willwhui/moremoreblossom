...
export class Game {
  ...
  init() {
    ...
    // Load birds from manifest (generated by vite-plugin-bird-manifest), then start the loop
    ...
    fetch('/bird-manifest.json')
      ...
      .then((manifest) => {
        ...
        return Promise.all(
          manifest.map((entry) => load(entry.glb).then((gltf) => ({ gltf, entry })))
        );
      })
      .then((results) => {
        // Pass both the gltf data and the manifest metadata to the Player
        const gltfs   = results.map((r) => r.gltf);
        const configs = results.map((r) => r.entry);
        this.player = new Player(this.scene, this.camera, this.inputManager, gltfs, configs);
        document.getElementById('loading').style.display = 'none';
        this.renderer.setAnimationLoop(() => this.animate());
      })
      .catch((err) => {
        console.error('Bird loading failed:', err);
        this.player = new Player(this.scene, this.camera, this.inputManager, null, null);
        document.getElementById('loading').style.display = 'none';
        this.renderer.setAnimationLoop(() => this.animate());
      });
  }
